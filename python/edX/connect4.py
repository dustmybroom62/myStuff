...
def check_piece(cell, piece):
    return cell is not None and piece == cell


def row_count(board, piece, row, col, cols):
    t = 0
    r = row
    c = col
    while c >= 0:
        if not check_piece(board[r][c], piece):
            break
        t += 1
        c -= 1
    c = col + 1
...
def col_count(board, piece, row, col, rows):
    t = 0
    r = row
    c = col
    while r >= 0:
...
def left_diag_count(board, piece, row, col, rows, cols):
    t = 0
    r = row
    c = col
    while r >= 0 and c >= 0:
...
def right_diag_count(board, piece, row, col, rows, cols):
    t = 0
    r = row
    c = col
    ...
    for r in range(rows):
        ...
        for c in range(cols):
            ...
            if piece is not None:
                if 3 < row_count(board, piece, r, c, cols):
                    return piece
                if 3 < col_count(board, piece, r, c, rows):
                    return piece
                if 3 < left_diag_count(board, piece, r, c, rows, cols):
                    return piece
                if 3 < right_diag_count(board, piece, r, c, rows, cols):
                    return piece
    return None
