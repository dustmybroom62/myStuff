...
def getAceTotal(total, aces):
    ...
    if 22 > total + (n - 1) + 11:
        return 11 + getAceTotal(total + 11, aces[1:])
    else:
        return 1 + getAceTotal(total + 1, aces[1:])
...
def getPoints(hand):
    ...
    total += getAceTotal(total, aces)
    return total

def next_move(hand):
    total = getPoints(hand)
    if 21 < total:
        return "Bust"
    if 17 > total:
        return "Hit"
